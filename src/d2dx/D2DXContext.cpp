	...
	D2Function d2Function)
{
	...
	if (_gameHelper->GetVersion() == GameVersion::Lod114d)
	{
		...
		// Bright white -> white
		// KOOKIE: more offending color text seqs.
		static const wchar_t offenders[] = L"/#%'\x06\x07\x09\x0c";
		static const wchar_t replaces[] = L"00001<$5";   // white*4 + coral(red), sage(green), teal(turquoise), gray
		...
		for (wchar_t* look = str; look <= end3 ; look++) {
			if (look[0] == L'\xff' && look[1] == L'c'
				...
				look[2] = replaces[opch - offenders];
		}
	}
